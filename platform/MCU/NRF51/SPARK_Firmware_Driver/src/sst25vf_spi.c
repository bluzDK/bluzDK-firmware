...
void sFLASH_EraseSector(uint32_t SectorAddr)
{
  ...
  /* Deselect the FLASH: Chip Select high */
  sFLASH_CS_HIGH();
  /* Wait for the busy status to clear */
...
/**
  * @brief  Sets the slave select Low, which selects the Flash as SPI slave.
  *
  * @retval None.
  */
static void sFLASH_CS_LOW(void)
...
/**
  * @brief  Sets the slave select High, which deselects the Flash as SPI slave.
  *
  * @retval None.
  */
static void sFLASH_CS_HIGH(void)
...
void sFLASH_WaitForWriteEnd(void)
{
  ...
  while ((flashstatus & sFLASH_WIP_FLAG) == 0x01); /* Write in progress */
...
int sFLASH_WriteSingleByte(uint32_t FLASH_Address, uint8_t byteToSend)
{
...
uint8_t sFLASH_ReadSingleByte(uint32_t FLASH_Address)
{
	...
	/* Read data from SPI FLASH memory */
	sFLASH_ReadBuffer(Rx_Buffer, FLASH_Address, BufferSize);
	return Rx_Buffer[0];
}
